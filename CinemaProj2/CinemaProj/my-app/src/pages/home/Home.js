...
export const Home = () => {
    ...
    const fetchMovies = async () => {
        try {
            const {results, total_pages, page, total_results} = await movieService.getMovies();
            return results;
        } catch (e) {
            console.error(e)
        }
    ...
    const fetchMoviesData = async () => {
        const requests = await [fetchMovies(), fetchGenres()]
        try {
            setIsLoading(true)
    ...
    return (
        <div>
            {isLoading || isLoading === null ? renderLoadingIndicator() : (<FilmList onFilmClick={onFilmClick}
                                                                                     items={moviesList}/>)}
        </div>
